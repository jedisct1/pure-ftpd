...
# endif
    ...
    free(LOCAL_chroot_base);
    LOCAL_chroot_base = NULL;
    if (path[0] == '/' && path[1] == 0) {   /* chroot("/") => no chroot */
        return 0;
    }
    if ((LOCAL_chroot_base = strdup(path)) == NULL) {
        return -1;
    }
    simplify(LOCAL_chroot_base);
    z = LOCAL_chroot_base;
    while (*z != 0) {
        z++;
    }
    for (;;) {
        z--;
        if (z == LOCAL_chroot_base || *z != '/') {
            break;
        }
        *z = 0;
    }
    if ((LOCAL_chroot_len = strlen(LOCAL_chroot_base)) >= sizeof LOCAL_curdir) {
# ifdef ENAMETOOLONG
        errno = ENAMETOOLONG;
# endif
        return -1;
...
char *fakegetcwd(char *dir, size_t size)
{
    ...
    if (LOCAL_chroot_base == NULL) {
        return getcwd(dir, size);
    }
    if (strncmp(LOCAL_curdir, LOCAL_chroot_base, LOCAL_chroot_len) != 0) {
        abort();
    }
    curdirchr = LOCAL_curdir + LOCAL_chroot_len;
    s = strlen(curdirchr);
    if (s <= (size_t) 0U) {
# ifdef EFAULT
        errno = EFAULT;
...
static int fakexlate(char *curdirtmp, size_t sizeof_curdirtmp, const char *dir)
{
    ...
    if ((curdirlen = strlen(LOCAL_curdir)) >= sizeof_curdirtmp) {
        return -1;
    }
    memcpy(curdirtmp, LOCAL_curdir, curdirlen + (size_t) 1U);
    simplify(curdirtmp);
    loop:
    if (dir[0] == '.' && dir[1] == '.' &&
        (dir[2] == 0 || dir[2] == '/')) {
        ...
        if (strncmp(curdirtmp, LOCAL_chroot_base, LOCAL_chroot_len) != 0 ||
            curdirtmp[LOCAL_chroot_len] != '/') {
            snprintf(curdirtmp, sizeof_curdirtmp, "%s/", LOCAL_chroot_base);
        }
...
int fakechdir(const char *dir)
{
    ...
    if ((curdirtmplen = strlen(curdirtmp)) >= sizeof LOCAL_curdir) {
        return -1;
    }
    memcpy(LOCAL_curdir, curdirtmp, curdirtmplen + (size_t) 1U);
