...
int
getnameinfo(const struct sockaddr *sa_, socklen_t salen, char *host,
            size_t hostlen, char *serv, size_t servlen, int flags)
{
    ...
    if (host != NULL && hostlen > (size_t) 1U) {
        ...
        if ((flags & NI_NUMERICHOST) == 0 &&
            (he = gethostbyaddr((const char *) &(sa->sin_addr),
...
{
    const struct sockaddr_in *const si = (const struct sockaddr_in *) ss;
...
{
    const struct sockaddr_in6 *const si = (const struct sockaddr_in6 *) ss;
